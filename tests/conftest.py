...
async def auto_cleanup_async_resources() -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        if connector and hasattr(connector, '_conns'):
            for conn_key, conns in list(connector._conns.items()):
...
def prevent_environment_pollution() -> Generator[None, None, None]:
    ...
    try:
        ...
        # Directory was deleted during test, that's ok
        pass
...
def cleanup_temp_files() -> Generator[None, None, None]:
    ...
    with patch('tempfile.mktemp', tracking_mktemp), \
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        "markers", "llm: Tests requiring LLM API access"
    )
    config.addinivalue_line(
        "markers", "benchmark: Performance benchmark tests"
    )
    config.addinivalue_line(
        "markers", "isolation: Tests requiring complete isolation"
    )
...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    ...
    for item in items:
        ...
        if "slow" in item.nodeid.lower() or getattr(item.function, "_slow_test", False):
