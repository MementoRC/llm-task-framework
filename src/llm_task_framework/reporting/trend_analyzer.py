...
class TrendDataStore:
    ...
    def store_benchmark_data(self, benchmark_data: list[dict[str, Any]], timestamp: str | None = None) -> None:
    ...
    def get_benchmark_history(self, benchmark_name: str, limit: int | None = None) -> list[dict[str, Any]]:
...
class TrendChartGenerator:
    ...
    def generate_trend_chart(self, test_name: str, historical_data: list[dict[str, Any]], 
                           metric: str = "mean", width: int = 12, height: int = 6) -> str:
        ...
        # Main trend line
        ax.plot(timestamps, values, 'b-', linewidth=2, label=f'{metric.title()} Execution Time', marker='o')
        
        # Confidence interval (if stddev available and metric is mean)
        if metric == "mean" and any(s > 0 for s in stddevs):
            upper_bound = [v + s for v, s in zip(values, stddevs)]
            lower_bound = [v - s for v, s in zip(values, stddevs)]
            ax.fill_between(timestamps, lower_bound, upper_bound, 
        ...
        if len(values) >= 2:
            ...
            if recent_avg > older_avg * 1.1:
                ax.text(0.02, 0.98, '📈 Trend: Degrading', transform=ax.transAxes, 
                       verticalalignment='top', color='red', fontweight='bold')
            elif recent_avg < older_avg * 0.9:
                ax.text(0.02, 0.98, '📉 Trend: Improving', transform=ax.transAxes, 
                       verticalalignment='top', color='green', fontweight='bold')
            else:
                ax.text(0.02, 0.98, '➡️ Trend: Stable', transform=ax.transAxes, 
    ...
    def _generate_no_data_chart(self, test_name: str, width: int, height: int) -> str:
        ...
        ax.text(0.5, 0.5, f'No historical data available\nfor {test_name}', 
...
class TrendAnalyzer:
    ...
    def analyze_trends(self, benchmark_names: list[str] | None = None, 
                      metric: str = "mean", history_limit: int = 50) -> dict[str, Any]:
        ...
        for benchmark_name in benchmark_names:
            history = self.data_store.get_benchmark_history(benchmark_name, history_limit)
    ...
    def _analyze_single_trend(self, history: list[dict[str, Any]], metric: str) -> dict[str, Any]:
        ...
        older_values = values[-older_count:] if len(values) > recent_count else values[recent_count:]  # Oldest
        ...
        if len(older_values) == 0:
            ...
            older_values = values[split_point:] if split_point < len(values) else [values[-1]]
