...
def load_benchmark_results(benchmark_file: Optional[Path]) -> Optional[Dict]:
...
def create_sample_data() -> tuple[TestExecutionResults, CoverageData, ExecutionTimes, SecurityResults]:
...
def main() -> None:
    ...
    if args.sample_data:
        ...
        test_results, coverage_data, execution_times, security_results = create_sample_data()
        ...
        execution_times = ExecutionTimes(0, 0, 0, 0, 0, 0)
        security_results = SecurityResults()
        ...
        if args.execution_times:
            try:
                if args.execution_times.startswith("{"):
                    ...
                    # File path
                    with open(args.execution_times) as f:
        ...
        if args.reports_dir.exists():
            security_results = reporter.load_security_results_from_reports(args.reports_dir)
